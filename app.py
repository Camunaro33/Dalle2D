...
# Hypothesis: simplified CSA A23.3 / ACI 318-style interior-column screen —
# vc = 0.33*lambda*phi_c*sqrt(f'c), critical perimeter at d/2 from the loaded
# area, lambda=1.0 (normal-density concrete). Only the single largest (factored)
# point load is checked; overlapping perimeters from closely-spaced multiple
# loads and edge/corner conditions are NOT accounted for.
...
if punching_applicable:
    ...
    vc_MPa = 0.33 * 1.0 * phi_c_punching * np.sqrt(fc_MPa)
    ...
    P_max_design_N = b0_mm = vc_MPa = Vc_kN = Vf_kN = 0.0
    punching_ok = True
...
if punching_applicable:
    if punching_ok:
        st.success(f"Punching shear (governing point load) — PASS: Vf = {Vf_kN:.0f} kN ≤ "
                   f"Vc = {Vc_kN:.0f} kN (b0 = {b0_mm:.0f} mm, d = {d_mm:.0f} mm).")
    else:
        st.error(f"Punching shear (governing point load) — NOT REALISTIC: Vf = {Vf_kN:.0f} kN "
                 f"exceeds Vc = {Vc_kN:.0f} kN (b0 = {b0_mm:.0f} mm, d = {d_mm:.0f} mm) — increase "
                 "thickness, increase the loaded-area size, or reduce the point load.")
else:
...
st.caption(
    "**Hypothesis for the punching shear check** — simplified interior-column screen: "
    f"vc = 0.33·λ·φc·√f'c with λ=1.0, φc={phi_c_punching:.2f}, f'c={fc_MPa:.0f} MPa; critical "
    f"perimeter b0 at d/2 from a {c1_mm:.0f}×{c2_mm:.0f} mm loaded area; only the single largest "
    f"{'factored ' if apply_uls else ''}point load ({P_max_design_N/1e3:.0f} kN) is checked. Does "
    "not account for edge/corner conditions, unbalanced-moment transfer, or overlapping critical "
    "perimeters from closely-spaced multiple loads.")
